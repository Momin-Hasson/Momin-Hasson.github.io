...
function renderProjectGrid() {
  ...
  container.innerHTML = projects
    .map(({ title, cardImage, Previewlink, Githublink }) => {
      const previewLink = Previewlink
        ? `<a class="btn btn--ghost" href="${Previewlink}" target="_blank" rel="noopener">Preview</a>`
        : "";
      const githubLink = Githublink
        ? `<a class="btn btn--ghost" href="${Githublink}" target="_blank" rel="noopener">GitHub</a>`
        : "";
      return `
      <div class="project-card card" data-reveal>
        <img class="project-card__image" src="${cardImage}" alt="${title}" loading="lazy">
        <div class="project-card__body">
          <h3 class="project-card__title">${title}</h3>
          <div class="project-card__links">${previewLink}${githubLink}</div>
        </div>
      </div>`;
    })
    .join("");
